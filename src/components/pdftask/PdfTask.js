            ...
            <table className='table-fixed border-collapse  border-2 border-gray-300'>
                <thead>
                    <tr className='border bg-gray-300 font-bold'>
                        ...
                        <td className='border p-1 text-center w-32 h-16' rowSpan={2}>Dose</td>
                        <td className='border p-1 text-center w-32 h-16' rowSpan={2}>Route</td>
                        <td className='border p-1 text-center w-32 h-16' rowSpan={2}>Frequency</td>
                        <>
                            {dataDetails.MedicationChartDetails.map((chartDetails) => (
                                <>
                                    {chartDetails.medicationChartDateDtoList.map((dateKey) => (
                                        <td
                                            colSpan={dateKey.medicationChartTimeDtoList.length}
                                            className='whitespace-nowrap text-center border w-32 h-16'
                                        >
                                            {dateKey.date}
                                        </td>
                                    ))}
                ...
                <tbody>
                    {dataDetails.MedicationChartDetails.map((chartDetails) => (
                        <tr>
                            <td className='border whitespace-nowrap text-center p-1 w-32 h-16'>
                                {chartDetails.prescDateTime}
                            </td>
                            <td className='border whitespace-nowrap text-center p-1 w-32 h-16'>
                            ...
                            <td className='border whitespace-nowrap text-center p-1 w-32 h-16'>
                                {chartDetails.frequency}
                            </td>
                            {chartDetails.medicationChartDateDtoList.map((dateKey) => (
                                <>
                                    {dateKey.medicationChartTimeDtoList.map((timeKey, index) => (
                                        <>
                                            {timeKey.adminDoneDateTime !== "" ? (
                                                <td
                                                    className='border text-center p-2 w-32 h-16'
                                                    key={index}
                                                >
                                                    <input type="checkbox" checked={timeKey.isDrugAdminDone} />
                                                    <h1>{timeKey.adminDoneDateTime}</h1>
                                                    <h1>{timeKey.adminDoneBy}</h1>
                                                </td>
                                            ) : (
                                                <td className='border text-center w-32 h-16'>-</td>
                                            )}
                                        </>
                                    ))}
                                </>
